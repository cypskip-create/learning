...
const TradingModel: React.FC<TradingModelProps> = ({
  stock,
  isOpen,
  onClose,
  ...
  // ✅ Ensure safe fallbacks for symbol/name
  ...
  const handleTrade = () => {
    try {
      if (tradeType === 'buy') {
        ...
        buyStock(symbol, shares, executePrice); // ✅ always a string
        setNotification({
          type: 'success',
        ...
        sellStock(symbol, shares, executePrice); // ✅ always a string
        setNotification({
          type: 'success',
  ...
  const handleSharesChange = (value: string) => {
    ...
    if (numValue >= 0) setShares(numValue);
  };
        ...
        <div className="modal-body">
          ...
          <div className="input-group">
            ...
            {tradeType === 'sell' && holding && (
              <span className="available-shares">
                Available: {holding.shares} shares
              </span>
            )}
            {tradeType === 'buy' && (
              <span className="available-shares">
                Available balance: KES {balance.toFixed(2)}
              </span>
            )}
          </div>
          ...
          {tradeType === 'buy' && !canBuy && shares > 0 && (
            <div className="warning-message">
              ⚠️ Insufficient balance. You need KES{' '}
              {(totalCost - balance).toFixed(2)} more.
            </div>
          )}
          {tradeType === 'sell' && !canSell && shares > 0 && (
            <div className="warning-message">
              ⚠️ You don't have enough shares. You only have{' '}
